...
public:
    ...
    static std::shared_ptr<mcfile::nbt::CompoundTag> Read(std::filesystem::path const& input, int32_t mapId) {
        using namespace std;
        namespace fs = std::filesystem;
        using namespace mcfile::stream;

        auto jeFilePath = input / "data" / ("map_" + to_string(mapId) + ".dat");
        if (!fs::is_regular_file(jeFilePath)) return nullptr;
...
private:
    static std::unordered_map<int32_t, int8_t> CreateScaleLookupTable(std::filesystem::path const& input) {
        namespace fs = std::filesystem;
