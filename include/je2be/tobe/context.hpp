...
struct Context {
  Context(JavaEditionMap const &mapInfo,
          ...
          bool allowCommand,
          std::function<CompoundTagPtr(Pos3i const &, mcfile::je::Block const &, CompoundTagPtr const &, Context const &)> fromBlockAndTileEntity) : fMapInfo(mapInfo), fWorldData(wd), fGameTick(gameTick), fDifficultyBedrock(difficultyBedrock), fAllowCommand(allowCommand), fFromBlockAndTileEntity(fromBlockAndTileEntity) {}
