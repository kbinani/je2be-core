...
public:
  ...
  static std::shared_ptr<Context> Convert(mcfile::Dimension d, std::unordered_set<Pos2i, Pos2iHasher> chunks, int rx, int rz, leveldb::DB *db, std::filesystem::path destination, Context const &parentContext) {
    ...
    for (int cz = rz * 32; cz < rz * 32 + 32; cz++) {
      ...
      for (int cx = rx * 32; cx < rx * 32 + 32; cx++) {
        ...
        auto j = Chunk(d, cx, cz, *b, *cache, *ctx);
  ...
  static std::shared_ptr<mcfile::je::WritableChunk> Chunk(mcfile::Dimension d, int cx, int cz, mcfile::be::Chunk const &b, ChunkCache<3, 3> &cache, Context &ctx) {
    using namespace std;
    ...
    for (auto const &sectionB : b.fSubChunks) {
      ...
      auto sectionJ = SubChunk(*sectionB);
      if (!sectionJ) {
        return nullptr;
      }
  ...
  static std::shared_ptr<mcfile::je::chunksection::ChunkSection118> SubChunk(mcfile::be::SubChunk const &sectionB) {
    using namespace std;
