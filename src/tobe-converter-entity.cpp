...
#define M(__name) table->insert(std::make_pair("minecraft:" #__name, Monster))
    ...
    E(boat, C(EntityBase, Vehicle(), Impl::Boat("boat")));
    E(chest_boat, C(EntityBase, Vehicle(), ChestItemsFromItems, Impl::Boat("chest_boat")));
...
#pragma region Behaviors
  ...
  static void PersistentFromFromBucket(CompoundTag &c, CompoundTag const &tag, ConverterContext &ctx) {
    ...
    c["NaturalSpawn"] = Bool(!fromBucket);

    if (fromBucket) {
      c["Persistent"] = Bool(true);
    } else {
      auto persistenceRequired = tag.boolean("PersistenceRequired", false);
      c["Persistent"] = Bool(persistenceRequired);
    }
  }
...
std::optional<std::tuple<Pos3i, CompoundTagPtr, std::string>> Entity::ToTileEntityBlock(CompoundTag const &c) {
  return Impl::ToTileEntityBlock(c);
}

std::tuple<Pos3i, CompoundTagPtr, std::string> Entity::ToItemFrameTileEntityBlock(CompoundTag const &c, std::string const &name) {
