...
bool ChunkDataPackage::serialize(ChunkData &cd) {
  return fImpl->serialize(cd);
}

void ChunkDataPackage::updateAltitude(int x, int y, int z) {
  fImpl->updateAltitude(x, y, z);
}

void ChunkDataPackage::addTileBlock(int x, int y, int z, std::shared_ptr<mcfile::je::Block const> const &block) {
  fImpl->addTileBlock(x, y, z, block);
}

void ChunkDataPackage::addLiquidTick(int order, CompoundTagPtr const &pendingTick) {
  fImpl->addLiquidTick(order, pendingTick);
}

void ChunkDataPackage::addTileTick(int order, CompoundTagPtr const &pendingTick) {
  fImpl->addTileTick(order, pendingTick);
}
